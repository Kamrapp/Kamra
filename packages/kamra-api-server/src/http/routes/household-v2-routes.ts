...
function commandError(error: unknown): ReturnType<typeof json> { const code = error instanceof Error ? error.message : "stock_command_failed"; const duplicate = typeof error === "object" && error !== null && "code" in error && (error as { code?: unknown }).code === 11000; const status = code === "idempotency_conflict" || code === "operation_in_progress" || code === "stale_revision" || code === "household_concept_already_exists" || duplicate ? 409 : code.endsWith("_not_found") ? 404 : 500; return json(status, { error: duplicate ? "household_concept_already_exists" : code }); }
