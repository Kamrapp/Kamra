...
export class FeatureFlagService {
  ...
  async evaluate(
    ...
    try {
      ...
      const value: FeatureFlagEvaluation = {
        enabled: record?.enabled ?? definition.defaultValue,
        key,
      ...
      const value: FeatureFlagEvaluation = {
        enabled: definition.failureValue,
        key,
        source: "failure"
      };
      this.cache.set(key, { expiresAt: this.now() + this.cacheTtlMs, value });
  ...
  async update(input: {
    ...
    await this.store.appendAudit({
      ...
      id: `${input.key}:${updated.revision}:${input.updatedAt}`,
      key: input.key,
      newValue: updated.enabled,
      oldValue: current?.enabled ?? featureFlagDefinitions[input.key].defaultValue,
