...
function store(
  initial: Partial<
    Record<"allowControlledAlphaAccess" | "allowAutoTickingAllShoppingListEntries", boolean>
  > = {}
): FeatureFlagStore & { audits: unknown[]; records: Map<string, FeatureFlagRecord> } {
  const records = new Map(
    Object.entries(initial).map(([key, enabled]) => [
      key,
      {
  ...
  return {
    ...
    write: async (input) => {
      const record = {
        ...
        key: input.key,
        revision: (records.get(input.key)?.revision ?? 0) + 1,
...
describe("feature flag service", () => {
  it("uses stored values, defaults, bounded caching, and invalidation", async () => {
    let now = 0;
    const flags = store({ allowControlledAlphaAccess: true });
    const service = new FeatureFlagService(flags, () => now, 100);
    expect(await service.evaluate("allowControlledAlphaAccess")).toMatchObject({
      enabled: true,
      source: "stored"
    });
    ...
    now = 101;
    expect(await service.evaluate("allowControlledAlphaAccess")).toMatchObject({ enabled: false });
    service.invalidate();
    flags.records.get("allowControlledAlphaAccess")!.enabled = true;
    expect(await service.evaluate("allowControlledAlphaAccess")).toMatchObject({ enabled: true });
  });
  ...
  it("fails closed for alpha access and records old/new values", async () => {
    ...
    expect(flags.audits[0]).toMatchObject({
      oldValue: false,
      newValue: true,
      reason: "alpha rollout"
    });
