...
export class MongoProductComposerRepository {
  ...
  async createProductWithBatch(input: CreateProductWithBatchInput): Promise<{
    batchId: string;
    operationId: string;
    productGroupId: string | null;
    productId: string;
    ...
    return await runMongoTransaction(this.transactionClient, async (session) => {
      ...
      if (existing) {
        ...
        if (existing.status === "completed" && existing.resultIdentifiers)
          return {
            batchId: existing.resultIdentifiers["batchId"]!,
            operationId: input.operationId,
            productGroupId:
              existing.resultIdentifiers["productGroupId"] === "null"
                ? null
                : (existing.resultIdentifiers["productGroupId"] ?? null),
            productId: existing.resultIdentifiers["productId"]!
          };
      ...
      if (!productGroupId && input.group) {
        ...
        const group: ProductGroup = {
          ...
          revision: 0,
          status: "active",
          targetPolicy: input.group.targetPolicy ?? null,
          trackingUnit: input.group.trackingUnit,
          updatedAt: now,
        ...
        !(await this.groups.findOne(
          { householdId: input.householdId, id: productGroupId, status: "active" },
          { session }
        ))
      ) {
      ...
      const product: HouseholdProduct = {
        ...
        createdAt: now,
        createdByUserId: input.actorUserId,
        defaultTrackingUnit: input.product.defaultTrackingUnit ?? null,
        directAttributes: [],
        directConcepts: [],
        ...
        identityKind: "manual",
        identitySnapshot: {},
        note: input.product.note ?? null,
        productGroupId,
        revision: 0,
        status: "active",
        targetPolicy: input.product.targetPolicy ?? null,
        updatedAt: now,
        updatedByUserId: input.actorUserId
      };
      const batchId = `stock-batch:${input.householdId}:${input.operationId}`;
      const batch: StockBatch = {
        acquiredOn: input.batch.acquiredOn,
        ...
        createdAt: now,
        createdByUserId: input.actorUserId,
        expiryOn: input.batch.expiryOn ?? null,
        householdId: input.householdId,
        householdProductId: productId,
        id: batchId,
        originalQuantity: input.batch.originalQuantity,
      ...
      const operation: DomainOperation = {
        ...
        operationType: "product_composer.create_product_with_batch",
      ...
      await this.operations.updateOne(
        ...
        {
          $set: {
            resultIdentifiers: { batchId, productGroupId: productGroupId ?? "null", productId },
            status: "completed",
            updatedAt: now
          }
        },
