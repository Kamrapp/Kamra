...
export class MongoStockReadRepository {
  ...
  async getTarget(
    ...
    targetId: string,
    today: string
  ): Promise<StockTargetReadModel | null> {
    ...
    return {
      ...
      batches: batches.filter((batch) =>
        allocations.some((allocation) => allocation.stockBatchId === batch.id)
      ),
      target
    };
  }
  ...
  async getWorkspace(householdId: string, today: string): Promise<StockWorkspaceReadModel> {
    ...
    const targetModels = targets.map((target) => {
      const targetAllocations = allocations.filter(
        (allocation) => allocation.stockTargetId === target.id
      );
      const targetBatches = targetAllocations
        .map((allocation) => batchesById.get(allocation.stockBatchId))
        .filter((batch): batch is StockBatch => Boolean(batch));
      const targetProducts = [
        ...new Map(
          targetBatches
            .map((batch) =>
              batch.householdProductId
                ? ([batch.householdProductId, productById.get(batch.householdProductId)] as const)
                : null
            )
            .filter(
              (entry): entry is readonly [string, HouseholdProduct | undefined] =>
                entry !== null && Boolean(entry[1])
            )
        ).values()
      ] as HouseholdProduct[];
      return {
    ...
    const productsWithBatches = new Set(
      batches.flatMap((batch) => (batch.householdProductId ? [batch.householdProductId] : []))
    );
    return {
      allowExpiredItems: household?.allowExpiredItems ?? true,
      products,
      targets: targetModels,
      unassignedBatches: batches.filter((batch) => !allocatedBatchIds.has(batch.id)),
      unassignedProducts: products.filter((product) => !productsWithBatches.has(product.id))
    };
  }
}
