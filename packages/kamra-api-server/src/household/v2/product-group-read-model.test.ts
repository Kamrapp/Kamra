...
const group = (
  id: string,
  name: string,
  parentProductGroupId: string | null = null
): ProductGroup => ({
  createdAt: time,
  createdByUserId: "u",
  displayName: name,
...
const product = (id: string, name: string, productGroupId: string | null): HouseholdProduct => ({
...
const batch = (id: string, productId: string, quantity: number): StockBatch => ({
  acquiredOn: "2026-07-01",
  acquisitionSnapshot: { displayName: "Bread" },
...
describe("buildProductGroupWorkspace", () => {
  it("rolls Product stock into its Group and ancestors once", () => {
    const result = buildProductGroupWorkspace({
      ...
      batches: [batch("b1", "p1", 1), batch("b2", "p1", 1)],
      groups: [group("g1", "Bread"), group("g2", "White bread", "g1")],
      products: [product("p1", "Pilos white bread", "g2")],
      today: "2026-07-12"
    });
    expect(result.productGroups[0]?.aggregate).toMatchObject({
      availableQuantity: 2,
      state: "at_target"
    });
    expect(result.productGroups[0]?.childGroups[0]?.aggregate).toMatchObject({
      availableQuantity: 2,
      state: "at_target"
    });
  ...
  it("keeps Products without Groups visible and neutral without a target policy", () => {
    const result = buildProductGroupWorkspace({
      ...
      batches: [],
      groups: [],
      products: [product("p1", "Unassigned flour", null)],
      today: "2026-07-12"
    });
  ...
  it("orders expired batches first, then earliest expiry", () => {
    ...
    const result = buildProductGroupWorkspace({
      allowExpiredItems: true,
      batches: [noExpiry, soon, expired],
      groups: [],
      products: [product("p1", "Milk", null)],
      today: "2026-07-12"
    });
    expect(result.unassignedProducts[0]?.batches.map((entry) => entry.id)).toEqual([
      "expired",
      "soon",
      "none"
    ]);
