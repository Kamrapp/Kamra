...
describe("MongoStockTargetRepository", () => {
  it("creates, revision-checks, updates, and archives targets", async () => {
    const db = createFakeDb({ household_stock_targets: new FakeCollection<Record<string, unknown>>("household_stock_targets", [target as unknown as Record<string, unknown>]) }); const repository = new MongoStockTargetRepository(db);
    expect((await repository.update({ expectedRevision: 0, householdId: "h", id: "target", patch: { minimumQuantity: 3 }, updatedAt: target.updatedAt, updatedByUserId: "u" })).revision).toBe(1);
    await expect(repository.update({ expectedRevision: 0, householdId: "h", id: "target", patch: { minimumQuantity: 1 }, updatedAt: target.updatedAt, updatedByUserId: "u" })).rejects.toThrow("stale_revision");
    expect((await repository.archive({ expectedRevision: 1, householdId: "h", id: "target", updatedAt: target.updatedAt, updatedByUserId: "u" })).status).toBe("archived");
  });
});
