...
describe("MongoHouseholdProductRepository", () => {
  it("creates products and revision-checks reusable classification changes", async () => {
    ...
    const repository = new MongoHouseholdProductRepository(db);
    await repository.create(product);
    const updated = await repository.updateClassification({
      directAttributes: [{ key: "fat.1_5_percent", scope: "catalog" }],
      directConcepts: [{ key: "food.milk", scope: "catalog" }],
      expectedRevision: 0,
      householdId: "h",
      id: "product-1",
    ...
    expect(updated.classificationRevision).toBe(1);
    expect(updated.directConcepts).toEqual([{ key: "food.milk", scope: "catalog" }]);
    const renamed = await repository.updateIdentity({
  ...
  it("preserves direct concept references when a Product is created", async () => {
    const created: HouseholdProduct = {
      ...product,
      directConcepts: [{ key: "household-bread", scope: "household" }]
    };
    const repository = new MongoHouseholdProductRepository(createFakeDb());
    await repository.create(created);
    await expect(repository.get("h", created.id)).resolves.toMatchObject({
