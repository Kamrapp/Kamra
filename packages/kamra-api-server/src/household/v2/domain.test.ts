...
const criteria: AcceptanceCriteria = {
  requiredConceptsAll: [concept("pasta")],
  acceptedConceptsAny: [concept("spaghetti")],
  requiredAttributesAll: [attribute("gluten_free")],
...
describe("household v2 classification", () => {
  it("calculates sorted inclusive ancestry and explains a matching product", () => {
    const classification = classifyProduct(
      ...
      [{ kind: "is_a", child: concept("spaghetti"), parent: concept("pasta") }]
    );
    expect(classification.effectiveConcepts).toEqual([concept("pasta"), concept("spaghetti")]);
    expect(matchAcceptanceCriteria(criteria, classification)).toMatchObject({
      accepted: true,
  ...
  it("rejects self-links and cycles", () => {
    expect(() =>
      validateConceptRelations([
        { kind: "is_a", child: concept("pasta"), parent: concept("pasta") }
      ])
    ).toThrow("self-link");
    expect(() =>
      calculateEffectiveConcepts(
...
const batch = (
  ...
  id,
  householdId: "household",
  householdProductId: "household-product:milk",
  acquiredOn,
  expiryOn,
  ...
  unit: "ml",
  status: "available",
  acquisitionSnapshot: { displayName: id },
...
const allocation = (batchId: string, quantity: number): StockAllocation => ({
...
describe("household v2 stock rules", () => {
  it("converts compatible units and prevents overlap double counting", () => {
    ...
    expect(
      aggregateAvailableQuantity(
        target,
        [batch("a", "2026-07-20", 1500), batch("b", null, 1000)],
        [allocation("a", 1500)]
      )
    ).toBe(1.5);
  });
  ...
  it("orders dated batches before no-expiry batches with deterministic ties", () => {
    expect(
      orderBatchesForConsumption(
        [batch("b", null, 1), batch("a", "2026-07-20", 1), batch("c", "2026-07-20", 1)],
  ...
  it("plans partial multi-batch consumption and rejects insufficient stock", () => {
    const first = batch("first", "2026-07-20", 1000, "2026-07-01");
    const second = batch("second", null, 1000, "2026-07-02");
    expect(
      planConsumption(
        target,
        [first, second],
        [allocation("first", 1000), allocation("second", 1000)],
        1.5
      )
    ).toEqual([
      { batchId: "first", quantity: 1000, unit: "ml" },
      { batchId: "second", quantity: 500, unit: "ml" }
    ]);
    expect(() => planConsumption(target, [first], [allocation("first", 1000)], 2)).toThrow(
      "insufficient_stock"
    );
  });

  it("summarizes allocated quantity and combines shortage and expiry notices", () => {
    const milk = batch("milk", "2026-07-13", 1500);
    const summary = summarizeStockTarget(target, [milk], [allocation("milk", 1500)], "2026-07-11");
    expect(summary).toEqual({
      availableQuantity: 1.5,
      batchCount: 1,
      expiringBatchCount: 1,
      nextExpiryOn: "2026-07-13",
      noticeCodes: ["below_minimum", "expiring_soon"],
  ...
  it("validates target, batch, allocation, precision, and expiry invariants", () => {
    ...
    expect(() =>
      assertStockBatch({ ...batch("orphan", null, 1), householdProductId: null })
    ).toThrow("householdProductId is required");
    ...
    expect(() =>
      assertStockAllocation({ ...allocation("a", 1), allocatedQuantity: 1.0000001 })
    ).toThrow("positive");
  });
});
