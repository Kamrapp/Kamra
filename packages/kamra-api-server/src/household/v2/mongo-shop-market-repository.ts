...
export class MongoShopMarketRepository {
  ...
  async setupCollections(): Promise<void> {
    await Promise.all([
      ...
      this.markets.createIndex({ countryCode: 1, aliases: 1, status: 1 }, { name: "shop_markets_country_alias_status" })
    ]);
  }
  ...
  async list(countryCode?: string): Promise<ShopMarket[]> {
    return await this.markets.find(countryCode ? { countryCode, status: "active" } : { status: "active" }).sort({ displayName: 1 }).toArray();
  }
}
