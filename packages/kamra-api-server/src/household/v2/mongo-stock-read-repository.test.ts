...
describe("MongoStockReadRepository", () => {
  ...
  it("groups allocated batches by Household Product and leaves unallocated batches visible", async () => {
    ...
    const db = createFakeDb({
      ...
      household_stock_batches: new FakeCollection<Record<string, unknown>>(
        "household_stock_batches",
        [
          allocated as unknown as Record<string, unknown>,
      ...
      household_products: new FakeCollection<Record<string, unknown>>("household_products", [
  ...
  it("keeps a Product with no physical batches visible as unassigned", async () => {
    ...
    const db = createFakeDb({
      ...
      household_products: new FakeCollection<Record<string, unknown>>("household_products", [
        emptyProduct as unknown as Record<string, unknown>
      ])
    });
    const result = await new MongoStockReadRepository(db).getWorkspace("h", "2026-07-11");
