...
export class MongoStockCommandRepository {
  ...
  private readonly households: MongoCollectionLike<{
    id: string;
  ...
  async setupCollections(): Promise<void> {
    await Promise.all([
      this.operations.createIndex(
        { householdId: 1, id: 1 },
        { name: "household_domain_operations_household_id_unique", unique: true }
      ),
      this.operations.createIndex(
        { householdId: 1, requestFingerprint: 1 },
        { name: "household_domain_operations_household_fingerprint" }
      ),
      this.movements.createIndex(
        { operationId: 1 },
        { name: "household_stock_movements_operation_unique", unique: true }
      )
    ]);
  }
  ...
  async allocateBatch(input: {
    ...
    operationId: string;
    requestFingerprint: string;
  }): Promise<{ allocationId: string; operationId: string }> {
    return await runMongoTransaction(this.transactionClient, async (session) => {
      ...
      const existing = await this.operations.findOne(
        { householdId: allocation.householdId, id: input.operationId },
        { session }
      );
      if (existing) {
        if (existing.requestFingerprint !== input.requestFingerprint)
          throw new Error("idempotency_conflict");
        if (existing.status === "completed" && existing.resultIdentifiers?.["allocationId"])
          return {
            allocationId: existing.resultIdentifiers["allocationId"],
            operationId: input.operationId
          };
        throw new Error("operation_in_progress");
      }
      const batch = await this.batches.findOne(
        { householdId: allocation.householdId, id: allocation.stockBatchId },
        { session }
      );
      const target = await this.targets.findOne(
        { householdId: allocation.householdId, id: allocation.stockTargetId },
        { session }
      );
      ...
      if (
        !areUnitsCompatible(batch.unit, target.trackingUnit) ||
        !areUnitsCompatible(allocation.unit, target.trackingUnit)
      )
        throw new Error("incompatible_tracking_unit");
      if (allocation.allocatedQuantity !== batch.remainingQuantity)
        throw new Error("allocation_must_cover_full_batch");
      ...
      const match = matchAcceptanceCriteria(
        target.acceptanceCriteria,
        batch.classificationSnapshot
      );
      if (!match.accepted && allocation.acceptanceResult !== "overridden")
        throw new Error("stock_batch_does_not_match_criteria");
      const now = batch.updatedAt;
      await this.operations.insertOne(
        {
          actorUserId: allocation.updatedByUserId,
          createdAt: now,
          householdId: allocation.householdId,
          id: input.operationId,
          operationType: "stock_batch.allocate",
      ...
      await this.operations.updateOne(
        ...
        {
          $set: {
            resultIdentifiers: { allocationId: allocation.id },
            status: "completed",
            updatedAt: now
          }
        },
  ...
  async consume(input: {
    ...
    actorUserId: string;
    occurredAt: string;
  }): Promise<{ operationId: string; consumedQuantity: number }> {
    return await runMongoTransaction(this.transactionClient, async (session) => {
      const existing = await this.operations.findOne(
        { householdId: input.householdId, id: input.operationId },
        { session }
      );
      if (existing) {
        if (existing.requestFingerprint !== input.requestFingerprint)
        ...
        if (existing.status === "completed")
          return { operationId: input.operationId, consumedQuantity: input.requestedQuantity };
        throw new Error("operation_in_progress");
      }
      const target = await this.targets.findOne(
        { householdId: input.householdId, id: input.stockTargetId },
        { session }
      );
      ...
      const allocations = await this.allocations
        .find(
          { householdId: input.householdId, stockTargetId: target.id, status: "active" },
          { session }
        )
        .toArray();
      const batches = (
        await Promise.all(
          allocations.map((allocation) =>
            this.batches.findOne(
              { householdId: input.householdId, id: allocation.stockBatchId },
              { session }
            )
          )
        )
      ).filter((batch): batch is StockBatch => Boolean(batch));
      ...
      const plan = planConsumption(
        ...
        input.selectedBatchIds,
        household?.allowExpiredItems ?? true,
      ...
      await this.operations.insertOne(
        {
          ...
          id: input.operationId,
          operationType: "stock_target.consume",
          requestFingerprint: input.requestFingerprint,
          status: "started",
      ...
      for (const line of plan) {
        ...
        await this.batches.updateOne(
          { householdId: input.householdId, id: batch.id, revision: batch.revision },
          {
            $set: {
              remainingQuantity,
              revision: batch.revision + 1,
              status: remainingQuantity === 0 ? "depleted" : "available",
              updatedAt: input.occurredAt,
        ...
        await this.allocations.updateOne(
          { householdId: input.householdId, id: allocation.id, revision: allocation.revision },
          {
            $set: {
              allocatedQuantity: allocation.allocatedQuantity - line.quantity,
        ...
        await this.movements.insertOne(
          {
            actorUserId: input.actorUserId,
            createdAt: input.occurredAt,
            householdId: input.householdId,
            id: `movement:${input.operationId}:${batch.id}`,
            kind: "consumption",
      ...
      await this.targets.updateOne(
        { householdId: input.householdId, id: target.id, revision: input.expectedTargetRevision },
        {
          $set: {
      ...
      await this.operations.updateOne(
        ...
        {
          $set: {
            resultIdentifiers: { stockTargetId: target.id },
            status: "completed",
            updatedAt: input.occurredAt
          }
        },
  ...
  async correctBatch(input: {
    ...
    occurredAt: string;
    reasonCode?: string;
  }): Promise<{ batchId: string; operationId: string }> {
    return await runMongoTransaction(this.transactionClient, async (session) => {
      const existing = await this.operations.findOne(
        { householdId: input.householdId, id: input.operationId },
        { session }
      ...
      if (existing) {
        if (existing.requestFingerprint !== input.requestFingerprint)
          throw new Error("idempotency_conflict");
        if (existing.status === "completed")
          return { batchId: input.batchId, operationId: input.operationId };
        throw new Error("operation_in_progress");
      }
      const batch = await this.batches.findOne(
        { householdId: input.householdId, id: input.batchId },
        { session }
      );
      ...
      if (!Number.isFinite(input.resultingQuantity) || input.resultingQuantity < 0)
        throw new Error("invalid_correction_quantity");
      const allocation = await this.allocations.findOne(
        { householdId: input.householdId, stockBatchId: batch.id, status: "active" },
        { session }
      );
      ...
      await this.operations.insertOne(
        {
          ...
          householdId: input.householdId,
          id: input.operationId,
          operationType: "stock_batch.correct",
          requestFingerprint: input.requestFingerprint,
          status: "started",
          updatedAt: now
        },
      ...
      const expiryOn: string | null =
        input.expiryOn === undefined ? (batch.expiryOn ?? null) : input.expiryOn;
      await this.batches.updateOne(
        { householdId: input.householdId, id: batch.id, revision: batch.revision },
        {
          $set: {
      ...
      if (allocation)
        await this.allocations.updateOne(
          ...
          {
            $set: {
              allocatedQuantity: input.resultingQuantity,
              revision: allocation.revision + 1,
              updatedAt: now,
              updatedByUserId: input.actorUserId
            }
      ...
      await this.operations.updateOne(
        { householdId: input.householdId, id: input.operationId },
        { $set: { resultIdentifiers: { batchId: batch.id }, status: "completed", updatedAt: now } },
        { session }
      );
      ...
      await this.operations.insertOne(
        {
          ...
          householdId: input.householdId,
          id: input.operationId,
          operationType: "stock_batch.discard",
          requestFingerprint: input.requestFingerprint,
          status: "started",
          updatedAt: now
        },
  ...
  async acquireBatch(input: BatchAcquisitionInput): Promise<CommandResult> {
    return await runMongoTransaction(this.transactionClient, async (session) => {
      const existing = await this.operations.findOne(
        { householdId: input.batch.householdId, id: input.operationId },
        { session }
      );
      if (existing) {
        if (existing.requestFingerprint !== input.requestFingerprint)
          throw new Error("idempotency_conflict");
        if (existing.status === "completed" && existing.resultIdentifiers?.["batchId"])
          return { batchId: existing.resultIdentifiers["batchId"], operationId: input.operationId };
        throw new Error("operation_in_progress");
      }
      const now = input.batch.updatedAt;
      const operation: DomainOperation = {
        actorUserId: input.batch.updatedByUserId,
        createdAt: now,
        householdId: input.batch.householdId,
        id: input.operationId,
        operationType: "stock_batch.acquire",
        requestFingerprint: input.requestFingerprint,
        status: "started",
        updatedAt: now
      };
      ...
      const movement: StockMovement = {
        ...
        id: `movement:${input.operationId}`,
        kind: "acquisition",
        occurrenceAt: input.batch.acquiredOn,
        operationId: input.operationId,
      ...
      await this.operations.updateOne(
        ...
        {
          $set: {
            resultIdentifiers: { batchId: input.batch.id },
            status: "completed",
            updatedAt: now
          }
        },
