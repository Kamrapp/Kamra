...
describe("MongoShopMarketRepository", () => {
  it("creates and lists active markets by country", async () => {
    const repository = new MongoShopMarketRepository(
      createFakeDb({ shop_markets: new FakeCollection("shop_markets") })
    );
    ...
    expect(await repository.get("shop-market:lidl-hu")).toMatchObject({
      displayName: "Lidl Hungary"
    });
  });
});
