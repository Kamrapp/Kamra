...
export function validateConceptRelations(relations: readonly ProductConceptRelation[]): void {
  ...
  for (const relation of relations) {
    if (refKey(relation.child) === refKey(relation.parent))
      throw new Error("product concept self-link is not allowed");
    const parents = direct.get(refKey(relation.child)) ?? [];
    parents.push(relation.parent);
    direct.set(refKey(relation.child), parents);
  }
  for (const child of direct.keys())
    calculateEffectiveConcepts(
      [
        {
          scope: child.split(":")[0] as ProductConceptRef["scope"],
          key: child.slice(child.indexOf(":") + 1)
        }
      ],
      relations
    );
...
export function matchAcceptanceCriteria(
  ...
  const matchedAcceptedConcepts = criteria.acceptedConceptsAny.filter((ref) =>
    concepts.has(refKey(ref))
  );
  const matchedAcceptedAttributes = criteria.acceptedAttributesAny.filter((ref) =>
    attributes.has(refKey(ref))
  );
  const matchedExcludedAttributes = criteria.excludedAttributesAny.filter((ref) =>
    attributes.has(refKey(ref))
  );
  const missingRequiredConcepts = criteria.requiredConceptsAll.filter(
    (ref) => !concepts.has(refKey(ref))
  );
  const missingRequiredAttributes = criteria.requiredAttributesAll.filter(
    (ref) => !attributes.has(refKey(ref))
  );
  const accepted =
  ...
  return {
    accepted,
    matchedAcceptedAttributes,
    matchedAcceptedConcepts,
    matchedExcludedAttributes,
...
export function orderBatchesForConsumption(
  batches: readonly StockBatch[],
  policy: StockTarget["consumptionPolicy"]
): StockBatch[] {
  return [...batches].sort((a, b) => {
    if (policy === "earliest_expiry_first") {
      if (a.expiryOn === null && b.expiryOn !== null) return 1;
      if (a.expiryOn !== null && b.expiryOn === null) return -1;
      if (a.expiryOn !== b.expiryOn)
        return (a.expiryOn ?? "9999-12-31").localeCompare(b.expiryOn ?? "9999-12-31");
    }
...
export function planConsumption(
  ...
  if (!Number.isFinite(requestedQuantity) || requestedQuantity <= 0)
    throw new Error("invalid_consumption_quantity");
  ...
  const activeAllocations = allocations.filter(
    (allocation) =>
      ...
      allocation.stockTargetId === target.id &&
      batchesById.has(allocation.stockBatchId)
  );
  const selected = selectedBatchIds?.length
    ? activeAllocations.filter((allocation) => selectedBatchIds.includes(allocation.stockBatchId))
    : activeAllocations;
  const ordered = selectedBatchIds?.length
    ? selected.sort(
        (a, b) =>
          selectedBatchIds.indexOf(a.stockBatchId) - selectedBatchIds.indexOf(b.stockBatchId)
      )
    : orderBatchesForConsumption(
        selected.map((allocation) => batchesById.get(allocation.stockBatchId)!),
        target.consumptionPolicy
      ).flatMap((batch) =>
        activeAllocations.filter((allocation) => allocation.stockBatchId === batch.id)
      );
  ...
  for (const allocation of ordered) {
    ...
    const quantity = Math.min(
      available,
      convertQuantity(remaining, target.trackingUnit, allocation.unit)
    );
...
export function aggregateAvailableQuantity(
  ...
  return allocations
    .filter(
      (allocation) => allocation.status === "active" && allocation.stockTargetId === target.id
    )
    .reduce((total, allocation) => {
      const batch = batchesById.get(allocation.stockBatchId);
      return batch
        ? total +
            convertQuantity(allocation.allocatedQuantity, allocation.unit, target.trackingUnit)
        : total;
    }, 0);
}
...
export function summarizeStockTarget(
  target: StockTarget,
  batches: readonly StockBatch[],
  allocations: readonly StockAllocation[],
  today: string,
  allowExpiredItems = true
): StockTargetAggregate {
  const availableBatches = batches.filter((batch) => batch.status === "available");
  const availableQuantity = aggregateAvailableQuantity(
    target,
    availableBatches,
    allocations,
    allowExpiredItems,
  ...
  const allocatedBatchIds = new Set(
    allocations
      ...
      .map((allocation) => allocation.stockBatchId)
  );
  const targetBatches = availableBatches.filter((batch) => allocatedBatchIds.has(batch.id));
  const expiringBatchCount = targetBatches.filter(
    (batch) =>
      batch.expiryOn !== null &&
      batch.expiryOn !== undefined &&
      batch.expiryOn >= today &&
      batch.expiryOn <= addDays(today, target.expiryWarningDays)
  ).length;
  const datedExpiries = targetBatches
  ...
  const status =
    availableQuantity < target.minimumQuantity
      ? "below_minimum"
      : availableQuantity >= target.targetQuantity
        ? "at_target"
  ...
  return {
    ...
    expiringBatchCount,
    nextExpiryOn: datedExpiries[0] ?? null,
    noticeCodes: [
      status === "below_minimum" ? "below_minimum" : null,
      expiringBatchCount > 0 ? "expiring_soon" : null
    ].filter((code): code is "below_minimum" | "expiring_soon" => code !== null),
    status
  };
}
...
export function canUseHouseholdCapability(
  context: HouseholdCapabilityContext,
  capability: HouseholdCapability
): boolean {
  if (!context.isMember) return false;
  return (
    (capability !== "manage_members" && capability !== "void_history") || context.role === "owner"
  );
}
