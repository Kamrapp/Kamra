...
describe("MongoStockCommandRepository", () => {
  it("acquires a batch with one movement and replays an identical retry", async () => {
    ...
    const repository = new MongoStockCommandRepository(db, transactionClient);
    await repository.setupCollections();
    const input = { batch, operationId: "op-1", requestFingerprint: "fingerprint" };
    expect(await repository.acquireBatch(input)).toEqual({
      batchId: "batch-1",
      operationId: "op-1"
    });
    expect(await repository.acquireBatch(input)).toEqual({
    ...
    expect(db.__collections["household_stock_movements"]!.docs).toHaveLength(1);
    await expect(
      repository.acquireBatch({ ...input, requestFingerprint: "different" })
  ...
  it("allocates a full batch once and rejects a second active allocation", async () => {
    ...
    const repository = new MongoStockCommandRepository(db, transactionClient);
    await repository.acquireBatch({
      batch,
      operationId: "op-acquire",
      requestFingerprint: "acquire"
    });
    ...
    expect(
      await repository.allocateBatch({
        allocation,
        operationId: "op-allocate",
        requestFingerprint: "allocate"
      })
    ).toEqual({ allocationId: "allocation-1", operationId: "op-allocate" });
    await expect(
      repository.allocateBatch({
        allocation: { ...allocation, id: "allocation-2" },
        operationId: "op-allocate-2",
        requestFingerprint: "allocate-2"
      })
    ).rejects.toThrow("active_allocation_exists");
  });
  ...
  it("consumes across allocated batches and records movements", async () => {
    ...
    expect(
      await repository.consume({
        ...
        requestFingerprint: "consume",
        requestedQuantity: 1,
        stockTargetId: target.id
      })
    ).toEqual({ consumedQuantity: 1, operationId: "op-consume" });
