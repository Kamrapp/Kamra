...
export function generateTargetPolicyShoppingNeed(input: { aggregate: ProductStockAggregate; displayName: string; id: string; needId: string; ownerKind: "household_product" | "product_group"; policy: TargetPolicy }): ShoppingNeed | null {
  if (input.aggregate.availableQuantity >= input.policy.minimumQuantity) return null;
  return { acceptanceCriteriaSnapshot: { acceptedAttributesAny: [], acceptedConceptsAny: [], excludedAttributesAny: [], requiredAttributesAll: [], requiredConceptsAll: [] }, id: input.needId, ownerDisplayNameSnapshot: input.displayName, ownerId: input.id, ownerKind: input.ownerKind, plannedQuantity: input.policy.desiredQuantity - input.aggregate.availableQuantity, reasonCode: "below_minimum", revision: 0, state: "open", unit: input.policy.trackingUnit };
}
...
export function createAdHocShoppingNeed(input: { id: string; plannedQuantity: number; unit: ShoppingNeed["unit"] }): ShoppingNeed {
  ...
  return { acceptanceCriteriaSnapshot: { acceptedAttributesAny: [], acceptedConceptsAny: [], excludedAttributesAny: [], requiredAttributesAll: [], requiredConceptsAll: [] }, id: input.id, ownerId: null, ownerKind: "manual", plannedQuantity: input.plannedQuantity, reasonCode: "manual", revision: 0, state: "open", stockTargetId: null, unit: input.unit };
}
