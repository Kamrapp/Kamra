...
import {
  migrateLegacyClassification,
  type ProductAttributeAssignmentRecord,
  type ProductAttributeRecord,
  type ProductConceptAssignmentRecord,
  type ProductConceptRecord
...
export class MongoClassificationRepository {
  private readonly attributeAssignments: MongoCollectionLike<
    ProductAttributeAssignmentRecord & MigrationDocument
  >;
  private readonly attributes: MongoCollectionLike<ProductAttributeRecord & MigrationDocument>;
  private readonly conceptAssignments: MongoCollectionLike<
    ProductConceptAssignmentRecord & MigrationDocument
  >;
  ...
  async setupCollections(): Promise<void> {
    await Promise.all([
      ...
      this.attributes.createIndex(
        { key: 1 },
        { name: "product_attributes_key_unique", unique: true }
      ),
      this.relations.createIndex(
        { "child.scope": 1, "child.key": 1, "parent.scope": 1, "parent.key": 1 },
        { name: "product_concept_relations_edge_unique", unique: true }
      ),
      this.conceptAssignments.createIndex(
        { productId: 1, "concept.scope": 1, "concept.key": 1 },
        { name: "product_concept_assignments_product_concept_unique", unique: true }
      ),
      this.attributeAssignments.createIndex(
        { productId: 1, "attribute.scope": 1, "attribute.key": 1 },
        { name: "product_attribute_assignments_product_attribute_unique", unique: true }
      )
    ]);
  }
  ...
  async migrateLegacy(
  ...
  ): Promise<ClassificationMigrationReport> {
    ...
    await this.upsert(
      ...
      result.relations.map((relation) => ({
        ...relation,
        id: `relation:${relation.child.scope}:${relation.child.key}:${relation.parent.scope}:${relation.parent.key}`
      }))
    );
  ...
  private async upsert<T extends MigrationDocument>(
    ...
    await collection.bulkWrite(
      records.map((record) => ({
        replaceOne: { filter: { id: record.id } as Filter<T>, replacement: record, upsert: true }
      }))
    );
  }
}
