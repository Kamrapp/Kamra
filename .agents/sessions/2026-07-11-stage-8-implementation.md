...
- Issue: Cross-stage manual verification is now centralized in `scripts/stage11-mvp-manual-test.md`.
...
- Demo fixture: reseeding now adds two allocated milk Products under one Target, an expired-before-acquisition yogurt batch, and an unassigned no-expiry flour batch. The current consolidated runbook is `scripts/stage11-mvp-manual-test.md`.
...
- The consolidated runbook `scripts/stage11-mvp-manual-test.md` now includes the explicit existing-Product `Add stock` flow and the revised sequence.
...
- Do not close Stage 8 until the full cross-stage runbook at `scripts/stage11-mvp-manual-test.md` is confirmed or explicitly waived item by item.
...
- Manual verification was later consolidated into `scripts/stage11-mvp-manual-test.md` for the Product Group UI. It explicitly marks moving shopping selection checkboxes into the v2 table as an unfinished Stage 8 follow-up rather than a verified behavior.
