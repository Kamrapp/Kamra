...
- Product Concepts are separate classification vocabulary, not stock rows. The Concepts footer may create an empty household-local Concept ordered alphabetically. Product-to-Concept association/de-association belongs to the Product editor and keeps Batch snapshots immutable. A full drag-and-drop classifier remains post-MVP.
