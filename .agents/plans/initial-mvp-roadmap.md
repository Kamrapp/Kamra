...
| Stage 8 | Household-domain correctness and complete user-side shopping | Completed implementation; manual closeout owned by Stage 11 | Product Groups with optional Group/Product target policies, Product-owned Batches/Movements, Shopping Needs, household settings, migrations, localized Home/Manual surfaces, v2-driven list generation, editable shopping lines, and transactional purchase-to-Product/Batch finalization are implemented. Remaining evidence and bug fixes run through the single Stage 11 MVP runbook. |
| Stage 9 | Shop, catalogue, price, and purchase-ingestion connection | Implementation complete; closeout owned by Stage 11 | Shop Markets, Shop Products, Price Observations, matcher-driven Trip planning with override/skip, admin review, Trip persistence/completion, and Home Trip UI are implemented. Remaining configured/browser evidence and narrowly scoped bug fixes run through Stage 11; Stage 9 must not reimplement the basic household list or purchase-to-stock loop. |
| Stage 10 | Alpha 1.0 hardening | Implementation complete; configured/browser closure evidence pending | Applies the final domain language, preserves/exports/imports Crawl Snapshots, fixes confirmed parser/data defects, improves demonstrated change-locality problems, and prepares external Alpha review through one continuous sequence of independently committed slices. Baseline findings are recorded in `docs/alpha-hardening-baseline.md`; verified archive export, guarded cutover, read-only quality-audit tooling, Trip safety hardening, parser repair tooling, and release documentation are committed. |
| Stage 11 | Vertical-slice locality and integration contracts | Implementation complete; integrated manual closure pending | Organizes the most obvious capability boundaries, makes registered patterns such as feature flags reflective, and adds deterministic integration contracts while reusing narrowly triggered configured MongoDB smokes. The single Stage 8–11 runbook owns final browser/operator evidence. |
| MVP closure | Release validation and bounded polish | Planned after Stage 11 and Stage 10 evidence | Runs the complete Stage 8–11 matrix, closes anticipated failure classes, records waivers, and permits only low-risk visual/interaction polish that does not alter domain behavior. |
