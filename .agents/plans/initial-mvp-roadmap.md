...
| Stage 10 | Alpha 1.0 hardening | In implementation; baseline and archive export complete | Applies the final domain language, preserves/exports/imports Crawl Snapshots, fixes confirmed parser/data defects, improves demonstrated change-locality problems, and prepares external Alpha review through one continuous sequence of independently committed slices. Baseline findings are recorded in `docs/alpha-hardening-baseline.md`; verified raw archive export is committed and the next slice is final-domain-language maintenance. |
