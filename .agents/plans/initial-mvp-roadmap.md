...
| Stage 9 | Shop, catalogue, price, and purchase-ingestion connection | Final implementation slice remaining | Shop Markets, Shop Products, Price Observations, admin review, Trip persistence/completion, and Home Trip UI are implemented. The tested matcher/applicable-price service still needs wiring into Trip planning, followed by seed/compatibility verification; Stage 9 must not reimplement the basic household list or purchase-to-stock loop. |
