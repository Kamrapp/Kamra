...
- Inventory every legacy/final term across routes, collections/fields, contracts/files, UI/i18n, logs, seeds, and docs; run full tests/build/lint/typecheck, the end-to-end scenario, file/change-locality inventory, and query/index review. Record each finding with severity, owner, and the exact later step that owns it. The captured baseline is `docs/mvp/alpha-hardening-baseline.md`.
...
- **Implemented (2026-07-13):** README Alpha journey and safety boundary, `CONTRIBUTING.md`, `SECURITY.md`, `docs/mvp/alpha-operations.md`, and `docs/mvp/alpha-release-checklist.md` now describe the current Product Group/Shopping Trip runtime and clearly separate automated, configured, and browser evidence.
...
The cross-stage human verification source of truth is `scripts/mvp/stage11-mvp-manual-test.md`. Stage 10 closeout must update its evidence log there and must not duplicate the checklist elsewhere.
