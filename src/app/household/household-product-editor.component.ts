...
export class HouseholdProductEditorComponent {
  ...
  async saveGroup(): Promise<void> {
    ...
    if (
      !name ||
      !this.groupDraft.trackingUnit.trim() ||
      !this.validLimits(
        this.groupDraft.hasTarget,
        this.groupDraft.minimumQuantity,
        this.groupDraft.desiredQuantity
      )
    )
      return this.fail(this.loc.t("household.groupSaveInvalid"));
    this.saving.set(true);
    this.errorMessage.set("");
    const policy = this.groupDraft.hasTarget
      ? this.policy(
          this.groupDraft.minimumQuantity,
          this.groupDraft.desiredQuantity,
          this.groupDraft.trackingUnit
        )
      : null;
    const result = this.selectedGroupId()
      ? await this.service.updateProductGroup({
          displayName: name,
          expectedRevision: this.selectedGroupRevision(),
          groupId: this.selectedGroupId()!,
          householdId: this.householdId(),
          targetPolicy: policy,
          trackingUnit: this.groupDraft.trackingUnit.trim()
        })
      : await this.service.createProductGroup({
          displayName: name,
          householdId: this.householdId(),
          targetPolicy: policy,
          trackingUnit: this.groupDraft.trackingUnit.trim()
    ...
    if (result.status === "error")
      return this.fail(result.message ?? this.loc.t("household.groupSaveFailure"));
    this.logger.log(
      "info",
      `${this.selectedGroupId() ? "Product Group saved" : "Product Group created"}: ${name}`,
      { displayName: name, groupId: this.selectedGroupId() }
    );
    this.clearAll();
    this.changed.emit();
  ...
  async saveProduct(): Promise<void> {
    ...
    if (
      ...
      !this.validLimits(
        ...
        this.productDraft.minimumQuantity,
        this.productDraft.desiredQuantity
      )
    )
      return this.fail(this.loc.t("household.productSaveInvalid"));
    this.saving.set(true);
    this.errorMessage.set("");
    if (
      !this.product() &&
      !this.productDraft.productGroupId &&
      this.groupDraft.displayName.trim()
    ) {
      const groupResult = await this.service.createProductGroup({
        displayName: this.groupDraft.displayName.trim(),
        householdId: this.householdId(),
        targetPolicy: this.groupDraft.hasTarget
          ? this.policy(
              this.groupDraft.minimumQuantity,
              this.groupDraft.desiredQuantity,
              this.groupDraft.trackingUnit
      ...
      if (groupResult.status === "error" || !groupResult.productGroup?.id) {
        this.saving.set(false);
        return this.fail(
          groupResult.message ?? this.loc.t("household.groupCreateForProductFailure")
        );
      }
      this.productDraft.productGroupId = groupResult.productGroup.id;
    }
    const policy = this.productDraft.hasTarget
      ? this.policy(
          this.productDraft.minimumQuantity,
          this.productDraft.desiredQuantity,
          this.productDraft.trackingUnit.trim()
        )
      : null;
    const identitySnapshot = { gtin: this.productDraft.gtin || null };
    const result = this.product()
      ? await this.service.updateProductIdentity({
          defaultTrackingUnit: this.productDraft.trackingUnit.trim(),
          displayName: name,
          expectedRevision: this.product()!.revision,
          householdId: this.householdId(),
          productId: this.product()!.id,
          productGroupId: this.productDraft.productGroupId,
          targetPolicy: policy,
          note: this.productDraft.note || null,
          catalogProductId: this.productDraft.catalogProductId || null,
          identitySnapshot
        })
    ...
    this.saving.set(false);
    if (result.status === "error")
      return this.fail(result.message ?? this.loc.t("household.productSaveFailure"));
    this.logger.log(
      "info",
      `${this.product() ? "Household Product saved" : "Household Product created"}: ${name}`,
      {
        displayName: name,
        productGroupId: this.productDraft.productGroupId,
        productId: this.product()?.id
  ...
  async saveBatch(): Promise<void> {
    const name = this.productDraft.displayName.trim();
    if (!name || this.batchDraft.quantity <= 0 || !this.batchDraft.unit.trim())
      return this.fail(this.loc.t("household.batchSaveInvalid"));
    this.saving.set(true);
    this.errorMessage.set("");
    if (this.batch()) {
      const result = await this.service.correctBatch({
        acquiredOn: this.batchDraft.acquiredOn,
      ...
      this.saving.set(false);
      if (result.status === "error")
        return this.fail(result.message ?? this.loc.t("household.batchSaveFailure"));
      this.logger.log("info", `Stock Batch saved for ${name}`, {
        batchId: this.batch()!.id,
        productId: this.product()?.id
      });
      this.clearAll();
    ...
    if (!productId) {
      const created = await this.service.createProductWithBatch({
        batch: {
          ...
          displayName: name,
          expiryOn: this.batchDraft.expiryOn || null,
          originalQuantity: this.batchDraft.quantity,
          unit: this.batchDraft.unit.trim()
        },
        group: this.productDraft.productGroupId
          ? null
          : this.groupDraft.displayName.trim()
            ? {
                displayName: this.groupDraft.displayName.trim(),
        ...
        product: {
          ...
          note: this.productDraft.note || null,
          productGroupId: this.productDraft.productGroupId,
          targetPolicy: this.productDraft.hasTarget
            ? this.policy(
                this.productDraft.minimumQuantity,
                this.productDraft.desiredQuantity,
      ...
      if (created.status === "error")
        return this.fail(created.message ?? this.loc.t("household.productAndBatchCreateFailure"));
      this.logger.log("info", `Product and Stock Batch created: ${name}`, {
        displayName: name,
        productGroupId: this.productDraft.productGroupId
      });
      this.clearAll();
    ...
    const result = await this.service.createBatch({
      ...
      quantity: this.batchDraft.quantity,
      unit: this.batchDraft.unit.trim()
    });
    this.saving.set(false);
    if (result.status === "error")
      return this.fail(result.message ?? this.loc.t("household.batchSaveFailure"));
    this.logger.log("info", `Stock Batch created for ${name}`, { productId });
    this.clearAll();
    this.changed.emit();
  }
  ...
  canClearGroup(): boolean {
    return Boolean(
      ...
      this.groupDraft.displayName.trim() ||
      this.groupDraft.trackingUnit !== "count" ||
      this.groupDraft.hasTarget
    );
  }
  canClearProduct(): boolean {
    return Boolean(
      this.product() ||
      this.productDraft.displayName.trim() ||
      this.productDraft.gtin ||
  ...
  canClearBatch(): boolean {
    return Boolean(
      this.batch() ||
      this.batchDraft.quantity !== 0 ||
      this.batchDraft.expiryOn ||
      this.batchDraft.unit !== "count"
    );
  }
  ...
  private async loadGroups(): Promise<void> {
    ...
    const result = await this.service.loadProductGroups(householdId);
    if (result.status === "ok") {
      this.productGroups.set(result.productGroups ?? []);
      const selected = result.productGroups?.find(
        (candidate) => candidate.id === (this.product()?.productGroupId ?? this.group()?.group.id)
      );
      if (selected) {
        this.selectedGroupId.set(selected.id);
        this.groupDraft = {
          displayName: selected.displayName,
          desiredQuantity: selected.targetPolicy?.desiredQuantity ?? 0,
          hasTarget: Boolean(selected.targetPolicy),
          minimumQuantity: selected.targetPolicy?.minimumQuantity ?? 0,
  ...
  private selectedGroupRevision(): number {
    return this.productGroups().find((group) => group.id === this.selectedGroupId())?.revision ?? 0;
  }
...
function createGroupDraft(group?: HouseholdV2ProductGroup["group"]): GroupDraft {
  return {
    displayName: group?.displayName ?? "",
    desiredQuantity: group?.targetPolicy?.desiredQuantity ?? 0,
    hasTarget: Boolean(group?.targetPolicy),
    minimumQuantity: group?.targetPolicy?.minimumQuantity ?? 0,
    trackingUnit: group?.trackingUnit ?? "count"
  };
}
function createProductDraft(product: HouseholdV2Product | null): ProductDraft {
  return {
    catalogProductId:
      typeof product?.identitySnapshot?.["catalogProductId"] === "string"
        ? (product.identitySnapshot["catalogProductId"] as string)
        : "",
    desiredQuantity: product?.targetPolicy?.desiredQuantity ?? 0,
    displayName: product?.displayName ?? "",
    gtin:
      typeof product?.identitySnapshot?.["gtin"] === "string"
        ? (product.identitySnapshot["gtin"] as string)
        : "",
    hasTarget: Boolean(product?.targetPolicy),
    minimumQuantity: product?.targetPolicy?.minimumQuantity ?? 0,
    note: product?.note ?? "",
    productGroupId: product?.productGroupId ?? null,
    trackingUnit: product?.targetPolicy?.trackingUnit ?? product?.defaultTrackingUnit ?? "count"
  };
}
function createBatchDraft(unit = "count", batch?: HouseholdV2Batch | null): BatchDraft {
  return {
    acquiredOn: batch?.acquiredOn ?? new Date().toISOString().slice(0, 10),
